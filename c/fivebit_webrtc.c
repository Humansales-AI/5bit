...
#else
  SHA1((const unsigned char*)input, strlen(input), out);
#endif
}
...
static void handle(int fd, const char *data_dir) {
  ...
  if (strstr(buf, "Upgrade: websocket") || strstr(buf, "upgrade: websocket") ||
    ...
    unsigned char hash[SHA1_DIGEST_LEN]; sha1(combined, hash);
    char accept[64]; base64_encode(hash, SHA1_DIGEST_LEN, accept);
    http_101(fd, accept);
